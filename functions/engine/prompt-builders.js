// ═══════════════════════════════════════════════════════════════════════
// AI 리포트 프롬프트 엔진 — 서버 이관 (ANALYSIS_LOGIC_SERVER_MIGRATION.md "아직 남은 작업 3번")
//
// js/ai-analysis.js를 그대로 옮긴 것이다(로직 변경 없음 — 순수 이관 원칙). render*/wire*/save*
// 등 DOM을 직접 만지는 함수와 카드 HTML 빌더는 브라우저 전용이라 Node에서 절대 호출되지 않지만,
// diff 비교를 위해 지우지 않고 그대로 남겨뒀다(saju-calc.js·gwansang-classify.js와 같은 방식).
//
// ⚠️ 딱 3곳만 실제로 손을 댔다(아래 검색 가능한 표시 "// [서버 이관 수정]"):
//   원본은 관상 원형 DB를 클라이언트 캐시(CharacterAPI.ensureXXXCatalog → 전역 변수)로 참조했는데,
//   서버는 이미 해당 DB를 require()로 직접 들고 있어서 캐시를 새로 받아올 필요가 없다. 그 세 곳만
//   "await CharacterAPI.xxx()" 호출을 제거하거나 서버 쪽 동급 함수 호출로 바꿨다 — 그 외의 프롬프트
//   조립 로직(어떤 문구를 어떤 순서로 요청하는지) 자체는 전혀 바꾸지 않았다.
//
// Gemini API 실제 호출(재시도·키 순환 포함)은 이 파일 맨 아래 callGeminiDirect()에서 처리한다 —
// functions/index.js의 geminiProxy가 이미 쓰고 있는 것과 같은 로직을 그대로 가져왔다(중복 유지 —
// geminiProxy는 폐기 전까지 그대로 동작해야 하므로 지금은 공유 모듈로 빼지 않고 복제해뒀다. 폐기
// 시점에 정리 예정 — ANALYSIS_LOGIC_SERVER_MIGRATION.md 5번 8단계).
// ═══════════════════════════════════════════════════════════════════════
const archetypeDb = require('./archetype-db');
const characterDb = require('./character-db');
const { classifyCompatibility } = require('./compatibility-engine');
const { PART_DEF, PART_CONTENT } = require('./part-content-db');
const { CG_KO, JJ_KO, CG_OH, JJ_OH, calcSipseongAll, calcSinkangSinyak, calcYongsin,
  collectSajuInsightSummary, computeOhaeng, get12Unseong, lifelineStage,
  SIBIUNSEONG_MEANING, SIPSEONG_MEANING, SIBISINSAL_MEANING, GWIIN_MEANING,
  LIFELINE_STAGE_LABEL } = require('./saju-calc');
const { isForeheadReliable, getGwansangRatios, judgePartStatus, calcFaceOhaeng,
  calcSamjeongRatio, classifyAllFeaturesRuleBased, classifyEyeArchetypeRuleBased,
  classifyFaceArchetypeRuleBased } = require('./gwansang-classify');
const { TRAIT_LABEL_KO, FACE_TRAIT_BASELINE, TRAITS } = require('./trait-config');
const { EYE_ARCHETYPE_DB, FACE_ARCHETYPE_DB, FOREHEAD_TYPE_DB, EYEBROW_TYPE_DB,
...
// [서버 이관 수정] 원본은 여기서 클라이언트 캐시를 새로 받아왔지만(await CharacterAPI.ensureArchetypeCatalog()),
// 서버는 이 파일 맨 위에서 이미 archetype-db.js를 require()로 직접 들고 있어 받아올 필요가 없다.
...
- perceived_by_partner(${nameB}가 보는 ${nameA})와 perceived_by_me(${nameA}가 보는 ${nameB})는
  반드시 서로 다른 시선이어야 합니다 — 같은 성향 묘사를 주어만 바꿔서 반복하면 안 됩니다. ⚠️ 이 두
  항목은 "나"/"상대" 대신 항상 ${nameA}, ${nameB} 실제 이름으로 방향을 명확히 쓰세요 — 두 사람 다
  제3자(계정 주인이 자기 자신이 아닌 다른 두 사람의 궁합을 보는 경우)일 수도 있어서 "나"라는 표현
  자체가 누구를 가리키는지 알 수 없게 됩니다.
- mind_hacking(서로의 마음을 사로잡는 법)·expectation_vs_reality(서로 바라는 모습 vs 실제)도 특정
  한쪽을 "나"로 가정하지 말고, ${nameA}와 ${nameB} 양쪽 다에 해당하는 상호적인 이야기로 쓰세요.
...
// 2026-08-30 DB 이원화 1단계 — COMPATIBILITY_DB(compatibility-engine.js)가 서버로 옮겨갔다.
// [서버 이관 수정] 원본은 여기서 CharacterAPI.getRelation으로 네트워크 요청을 한 번 더 보냈지만,
// 서버는 같은 프로세스 안에 compatibility-engine.js를 이미 갖고 있어 classifyCompatibility를
// 직접 호출하면 된다(async 유지 — 호출부 buildGunghapUserPrompt가 이미 await로 부르고 있어서
// 시그니처를 안 바꾸는 게 안전하다).
async function buildGunghapRelationBlock(charIdA, charIdB) {
  if (!charIdA || !charIdB) return '';
  const rel = classifyCompatibility(charIdA);
  if (!rel) return '';
  let label = '내 사람';
...
async function buildDeepReportUserPrompt(
  ...
  // [서버 이관 수정] 원본은 여기서 클라이언트 캐시를 방어적으로 한 번 더 받아왔지만, 서버는 이
  // 파일 맨 위에서 character-db.js를 이미 require()로 갖고 있어 그럴 필요가 없다.
...
올해 세운 오행: ${sewoonInfo.yearOh} / 일간과의 관계: ${sewoonInfo.text}`
  ...
  // [서버 이관 수정] 같은 이유로 archetype-db.js 캐시 재요청 제거(이미 require()로 보유).
...
// [서버 이관 수정] 브라우저 부트스트랩 호출 — Node에는 DOM이 없어 무력화만 한다(saju-calc.js와 같은 방식).
if (typeof document !== 'undefined') renderGwansangRevisitCard();
...
// ═══ Gemini 실제 호출 (서버 전용) — functions/index.js geminiProxy의 키 순환·재시도 로직을 그대로
// 가져왔다(로직 동일, 중복 유지 — geminiProxy 폐기 전까지는 그쪽도 계속 동작해야 하므로 지금은
// 공유 모듈로 합치지 않았다). 클라이언트의 옛 callGeminiAPI(fetch(GEMINI_PROXY_URL, ...))가 하던
// "응답에서 text 뽑아 JSON.parse" 단계까지 여기서 한 번에 처리해서, 최종 파싱된 리포트 객체만
// 돌려준다 — 클라이언트는 이제 systemInstruction·스키마·Gemini raw 응답을 전혀 보지 않는다. ══
async function callGeminiDirect(apiKeys, systemInstruction, userText, images, schema, temperature = 0.9, model) {
  const keys = (apiKeys || []).filter(Boolean);
  if (!keys.length) throw new Error('Gemini API 키가 서버에 설정되지 않았습니다.');
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    try {
      ...
      if (!isRetryable) throw new Error(`Gemini API 오류 (${geminiRes.status}): ${JSON.stringify(data).slice(0, 200)}`);
      console.warn(`[callGeminiDirect] 키 ${i + 1}/${keys.length} 실패(${geminiRes.status}) — 다음 키로 재시도`);
      lastError = data;
    } catch (e) {
      if (e.message && e.message.startsWith('Gemini API 오류')) throw e; // 재시도 불가 오류는 즉시 전파
      console.warn(`[callGeminiDirect] 키 ${i + 1}/${keys.length} 호출 실패(${e.name}) — 다음 키로 재시도`);
      lastError = { error: e.message };
    } finally {
      clearTimeout(timeout);
    }
  }
  throw new Error(`등록된 모든 키로 시도했지만 실패했습니다. ${JSON.stringify(lastError).slice(0, 200)}`);
}

// ═══ 서버 진입점 3종 — functions/index.js가 이것들만 호출하면 된다 ═══
// 클라이언트는 지금까지처럼(computeSaju·classifyGwansang 응답 등으로) 필요한 재료를 조립해서
...
module.exports = {
  generateDeepReport,
  generateAiEnhancement,
  generateGunghapReport,
  ...
  buildAiEnhancementUserPrompt,
  AI_ENHANCEMENT_SCHEMA,
  buildGunghapSystemInstruction,
  buildGunghapUserPrompt,
  buildGunghapReportSchema,
};
