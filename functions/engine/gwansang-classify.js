// ═══════════════════════════════════════════════════════════════════════
// 관상 판정 엔진 — 서버 이관 (ANALYSIS_LOGIC_SERVER_MIGRATION.md "아직 남은 작업 1번")
//
// js/landmark-engine.js를 그대로 옮긴 것이다(로직 변경 없음 — 순수 이관 원칙). loadModels·
// runFaceAnalysis·toggleMirror·drawRegions는 MediaPipe 모델 로딩/캔버스 디버그 오버레이라
// 브라우저 전용이고, 랜드마크 478점 추출 자체는 이번 단계에서도 계속 클라이언트(브라우저 WASM)에
// 남는다(마이그레이션 문서 4.2 "1차 권장안" — MediaPipe는 구글이 공개한 범용 얼굴 인식 모델이라
// 이 서비스만의 "판정 기준"이 아니다). 이 파일에서 실제로 호출하는 건 랜드마크 좌표(lm 배열)를
// 받아서 "무엇으로 판정하는지"를 계산하는 함수들뿐 — classifyGwansangBundle 이하만 실제 진입점이다.
// 나머지(로딩·캔버스 코드)는 Node에서 절대 호출되지 않는 죽은 코드로, diff 비교가 되도록 지우지
// 않고 그대로 남겨뒀다.
//
// judgePartStatus·gwansangLevel·GWANSANG_FEATURE_RANGE·PART_KEY_TO_MEASURE는 원래 js/app.js
// (952~966행, 3347~3353행)에 있던 것을 같이 옮겼다 — getGwansangRatios가 반환한 ratios를 받아
// "부위별 강점/보완"을 판정하는, 이 파일과 밀접하게 붙어 있는 다음 단계라 분리하지 않았다.
// ═══════════════════════════════════════════════════════════════════════
...
// ═══ 서버 진입점 — 랜드마크 좌표(lm, MediaPipe 478점 배열)를 받아 관상 판정 결과 전체를 돌려준다 ═══
// 클라이언트(js/ai-analysis.js classifyAndBuildCharacter, 3063~3084행 부근)가 지금까지
// classifyAllFeaturesRuleBased(lm) → getGwansangRatios(lm) → judgePartStatus(ratios) 순서로
// 직접 호출해 조립한 뒤 CharacterAPI.analyzeCharacter로 보내던 것과 같은 모양(featureIds/
// confidences/partStatusMap)을 반환한다 — analyzeCharacter가 이미 그 필드명을 기대하고 있어서
// 그대로 이어 쓸 수 있다.
//
