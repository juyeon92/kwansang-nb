// 개인 리포트용 AI 보완 — 별도 리포트를 새로 만드는 게 아니라, 이미 화면에 있는 로컬 카드 안에
// "사진을 봐야만 알 수 있는" 한 문장을 부위별로 덧붙이는 용도. 그래서 스키마도 부위별 짧은 문장 배열뿐임.
// 고전 문헌(마의상법연구논문, 정민이 2019, 표Ⅲ-9·Ⅲ-10) 기반 눈모양·동물형상 명명 참고자료
// AI가 사진을 직접 보고 "이 눈은 봉안(鳳眼)에 가깝다" 식으로 구체적인 관상 용어를 짚어줄 수 있게 하는 근거 데이터.
// 39종/36종 원문 중 현대 독자에게 의미가 뚜렷하고 표현이 과하게 부정적이지 않은 것 위주로 선별함(초안).
// read/관상 형상 예시.md 전문을 그대로 옮긴 큐레이션 데이터 — AI는 분류(어떤 ID인지)만 하고,
// 설명 문장은 여기 미리 써둔 걸 그대로 보여준다(매번 다른 AI 문장이 아니라 검증된 고정 설명).
...
// 눈 아이콘 — 실제 눈 사진 대신, 유형별 형태 차이(너비/높이/기울기/눈꼬리)를 살린 일러스트(같은 스타일 유지)
...
// ═══ 부위별 세부 유형 DB — read/관상_MBTI_데이터정리 (1).md 를 옮긴 것 (MBTI 축/그래프는 전부 제외, 강점·약점만 사용) ═══
// 위 EYE_ARCHETYPE_DB(용안·봉안 등 전통 물형)와는 별개 축 — 여기는 크기/모양(큰눈·작은눈 등) 기준의 실용적 강약점 분류.
// AI는 사진을 보고 어떤 유형 ID에 가까운지만 고르고, 강점/약점 문구는 아래 고정값을 그대로 노출한다.
// FOREHEAD_TYPE_DB/EYEBROW_TYPE_DB 등 detail 필드는 "관상 가이드_*.pdf"(2026-08-13, 8종) 내용 중
// 기존 항목과 겹치지 않는 기준만 뽑아 보강한 것 — 톤은 걷어내고 판단 기준·의미만 반영(점/얼굴 점은 제외).
...
const EYE_SHAPE_DB = {
  ES_BIG: { nameKo:'큰 눈', strength:'적극적이고 개방적이에요. 감정 몰입도가 커서 사랑이든 일이든 화끈하게 빠져드는 편이에요.', weakness:'마음이 떠나면 미련 없이 돌아서기도 하고, 호언장담할 때가 있어요.', detail:'내 사람과 아닌 사람을 직감적으로 빠르게 구분해서 사람 보는 기준이 꽤 까다로운 편이에요. 새로운 사람을 만나도 처음엔 여럿이 함께하며 시간을 두다가, 한번 마음을 열면 급격히 가까워져요. 지시받기보다 스스로 판단해 움직이는 환경에서 리더십이 더 빛나는 타입이에요.' },
  ES_SMALL: { nameKo:'작은 눈', strength:'의지력과 근성이 좋아요. 서두르지 않고 꾸준히 매달려 결과를 만들어내는 타입이에요.', weakness:'자기주장이 강할 수 있고, 처음엔 매력이 확 드러나지 않아 알아가는 데 시간이 걸리는 편이에요.', detail:'인기를 서둘러 좇기보다 때를 기다리는 편이라 오히려 늦게 빛나요. 특히 눈이 작고 길게 뻗은 편이라면 세상을 보는 통찰력이 좋아서, 남들과 다른 독특한 발상으로 자기 영역을 만들어가는 힘이 있어요.' },
  ES_DROOPY: { nameKo:'처진 눈', strength:'인자하고 감수성이 풍부해요.', weakness:'잔머리를 쓸 수 있어요.', detail:'눈꼬리가 아래로 살짝 내려간 편이라 온화하고 정이 많은 성정으로 봐요. 남의 마음을 잘 헤아려 편하게 해주어 대인관계와 연애에서 호감을 사기 쉽지만, 마음이 여려 거절을 어려워하는 면이 있어 스스로를 지키는 지혜도 함께 지니면 좋아요.' },
  ES_MONOLID: { nameKo:'외꺼풀 눈', strength:'관찰력이 좋고 이론적이에요.', weakness:'질투심이 있을 수 있어요.', detail:'감정을 안으로 다스리는 성정이라 속이 깊은 편이에요. 마음을 여는 데는 시간이 걸리지만, 한번 마음을 열면 그 정이 오래가는 편이라고 봐요.' },
  ES_UPTURNED: { nameKo:'올라간 눈', strength:'자신감 있고 강직해요.', weakness:'차갑게 보일 수 있어요.', detail:'눈꼬리가 위로 향한 이른바 치켜뜬 눈이라, 기상이 뚜렷하고 자존심이 강한 성정으로 봐요. 승부욕과 추진력이 있어 뜻한 바를 밀고 나가는 힘이 있지만, 기가 세 보이는 인상을 줄 수 있어서 부드러운 표정을 곁들이면 매력이 한결 살아나요.' },
  ES_DOUBLE: { nameKo:'쌍꺼풀 눈', strength:'화려하고 임기응변이 좋아요.', weakness:'과시적으로 비칠 수 있어요.', detail:'감정 표현이 솔직하고 정이 겉으로 잘 드러나는 편이라, 연애에서도 애정을 아끼지 않는 스타일로 봐요.' },
  ES_WIDE_SET: { nameKo:'눈 사이가 넓은 눈(원거리안)', strength:'도량이 넓고 마음이 느긋해요. 사람을 너그럽게 품는 편이에요.', weakness:'긴장감이 필요한 순간에도 여유를 부릴 수 있어요.', detail:'⚠️ 이 유형은 판단 기준값이 실측 데이터가 아니라 통상적인 얼굴 비례론(오안비율) 벤치마크를 참고한 초안이에요. 다만 이런 얼굴형인 상대는 사람을 두루 품는 편이라 편하게 다가가도 좋아요.' },
  ES_CLOSE_SET: { nameKo:'눈 사이가 좁은 눈(근거리안)', strength:'집중력이 뛰어나고 한 가지에 몰두하는 힘이 좋아요.', weakness:'조급해지기 쉬우니 마음의 여유를 곁들이면 좋아요.', detail:'⚠️ 이 유형도 마찬가지로 실측 데이터가 아닌 통상적인 벤치마크 기준의 초안이에요.' },
};
const NOSE_SHAPE_DB = {
  NS_SMALL_SHORT: { nameKo:'작은 코·짧은 코', strength:'재치 있고 순발력이 좋아요.', weakness:'즉흥적으로 행동할 수 있어요.' },
  NS_WIDE: { nameKo:'넓은 코', strength:'대외 활동력과 생활력이 좋아요.', weakness:'지배욕이 있을 수 있어요.' },
  NS_AQUILINE: { nameKo:'매부리코', strength:'신중하고 처세술이 좋아요.', weakness:'인색해 보일 수 있어요.' },
  NS_BENT: { nameKo:'꺾인 코', strength:'개성 있고 직감력이 좋아요.', weakness:'계산적으로 비칠 수 있어요.' },
  NS_BIG: { nameKo:'큰 코', strength:'활동적이고 현실적이에요.', weakness:'융통성이 부족할 수 있어요.' },
  NS_UPTURNED: { nameKo:'올라간 코', strength:'사교적이고 다재다능해요.', weakness:'잘난 척으로 비칠 수 있어요.' },
  NS_ALAR_THICK: { nameKo:'도톰한 콧방울', strength:'들어온 재물을 잘 지키는 알뜰한 성정이에요. 씀씀이를 헤프게 하지 않아요.', weakness:'지나치게 아끼는 데 신경 쓰다 보면 재미가 덜할 수 있어요.' },
  NS_ALAR_THIN: { nameKo:'얇은 콧방울·콧구멍이 드러나는 코', strength:'통이 크고 나누는 데 아낌이 없어요.', weakness:'씀씀이가 큰 편이라 지출 관리가 필요해요.' },
  NS_BOKGO: { nameKo:'복코(콧대는 완만하고 준두·콧방울이 도톰한 코)', strength:'재물복과 원만한 성정을 함께 갖춘 후덕한 상이에요. 사람들과 두루 잘 어울려요.', weakness:'특별히 두드러지는 약점보다는, 콧대가 도드라지지 않아 존재감이 은근한 편이에요.' },
};
const MOUTH_SHAPE_DB = {
  MS_THICK: { nameKo:'두꺼운 입', strength:'정이 많고 성실해요.', weakness:'지출이 많을 수 있어요.' },
  MS_BIG: { nameKo:'큰 입', strength:'행동력과 결단력이 좋아요.', weakness:'잘 속을 수 있어요.' },
  MS_SMALL: { nameKo:'작은 입·윗입술 얇은 입', strength:'기획력이 있고 호기심이 많아요.', weakness:'지구력이 부족할 수 있어요.' },
  MS_THIN: { nameKo:'얇은 입·아랫입술 얇은 입', strength:'센스 있고 민첩해요.', weakness:'냉정하게 비칠 수 있어요.' },
  MS_DOWNTURNED: { nameKo:'양끝 처진 입·아랫입술 나온 입', strength:'투지력이 있고 신의를 지켜요.', weakness:'반항적일 수 있어요.', detail:'입꼬리가 처진 인상을 줄 수 있지만 이는 고정된 운이 아니라 그때그때의 기색에 가까워요. 성정 자체는 진중하고 신중한 편이라, 말을 아끼고 실속을 챙기는 기질로도 볼 수 있어요.' },
  MS_UPTURNED: { nameKo:'양끝 올라간 입·보통 입', strength:'명랑하고 긍정적이에요.', weakness:'마음이 약할 수 있어요.', detail:'입꼬리가 살짝 위로 향한 입은 관상에서 매우 길하게 여겨요. 늘 웃는 인상을 주어 사람을 끌어당기고, 대인관계와 애정운이 두루 좋다고 봐요.' },
};
const CHIN_SHAPE_DB = {
  CS_UNDERBITE: { nameKo:'주걱턱·갈라진 턱', strength:'정력적이고 대담해요.', weakness:'독불장군처럼 보일 수 있어요.', detail:'턱 가운데가 살짝 갈라진 편이면 개성이 강하고 매력 있는 상으로 여기며, 의지가 굳은 성정으로 풀이해요.' },
  CS_OVAL: { nameKo:'타원형 턱', strength:'꼼꼼하고 직감력이 좋아요.', weakness:'우울해질 때가 있어요.' },
  CS_ROUND: { nameKo:'둥근 턱', strength:'포용력이 있고 낙천적이에요.', weakness:'결단성이 부족할 수 있어요.', detail:'턱과 볼 아래쪽(노복궁)이 도톰하면 아랫사람·후배의 도움을 받는 복이 있고 따르는 사람이 많다고 봐요. 귀 아래에서 턱으로 이어지는 볼살이 도톰하면 말년에 재물과 사람이 곁에 머무르는 상으로도 풀이해요.' },
  CS_SQUARE: { nameKo:'사각턱', strength:'집념이 있고 청렴해요.', weakness:'아집이 있을 수 있어요.' },
  CS_LONG: { nameKo:'긴 턱', strength:'원만하고 온후해요.', weakness:'자기 주관이 부족할 수 있어요.' },
  CS_POINTED: { nameKo:'뾰족한 턱', strength:'예술적이고 감각적이에요.', weakness:'자존심이 강할 수 있어요.' },
};
// 얼굴형 — 기존 관상오행(calcFaceOhaeng, 목화토금수 %)과는 다른 축이라 화면에는 "얼굴형 유형"으로 구분해 표기.
// coaching은 "이 얼굴형인 상대를 대할 때" 조언(원본 §8) — 궁합/관계 조언 콘텐츠로 재사용 가능.
const FACE_SHAPE_TYPE_DB = {
  FS_RECTANGLE: { nameKo:'직사각형', strength:'착하고 부드러우며 온화하고 인자해요. 희망적이고 추진력도 있어요.', weakness:'결단력이 부족하고 마무리가 약할 수 있어요.', coaching:'시간을 두고 대하는 게 좋아요. 인간적으로 대하되 논리적인 설명을 충분히 곁들여 보세요.' },
  FS_SQUARE: { nameKo:'정사각형', strength:'규칙적이고 모범적이며 책임감이 강해요. 신뢰를 잘 지켜요.', weakness:'융통성이 부족하고 배려가 더 필요할 수 있어요.', coaching:'"거북이를 상대한다"는 마음으로 느긋하게 신뢰를 쌓아보세요. 한번 친해지면 오래가는 관계가 돼요.' },
  FS_TRIANGLE: { nameKo:'삼각형', strength:'냉철하고 이성적이며 분석적이고 인내심이 좋아요.', weakness:'차갑게 보이거나 저항적일 수 있어요.', coaching:'존경을 표현하고, 충고보다는 논리적·수치적으로 조목조목 설명해보세요.' },
  FS_INV_TRIANGLE: { nameKo:'역삼각형', strength:'창조적이고 예술적이며 아이디어가 좋아요.', weakness:'이기적이거나 비현실적으로 비칠 수 있어요.', coaching:'감정을 이해하고 공감해주는 게 중요해요. 솔직하게 있는 그대로 표현하면 잘 통해요.' },
  FS_ROUND: { nameKo:'원형', strength:'외향적이고 사교적이며 사람을 좋아하고 따뜻해요.', weakness:'오지랖이 넓거나 다혈질일 수 있어요.', coaching:'인간적 교류를 우선하고, 맞장구쳐주며 칭찬해주면 좋아요. 무관심은 참지 못해요.' },
  FS_OVAL: { nameKo:'타원형', strength:'다재다능하고 순발력·적응력이 좋아요. 중재를 잘해요.', weakness:'잘난 척하거나 예민할 수 있어요.', coaching:'다양한 접근이 필요해요. 인간적 관계와 감성을 자극하는 아이디어로 다가가 보세요.' },
};
