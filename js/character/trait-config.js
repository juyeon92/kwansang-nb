...
// ⚠️ 2026-08-17 재보정 — 위 시뮬레이션 근사치가 실제 엔진 출력과 어긋나 있었다.
// 실측(사진 6장)으로 raw 분포를 역산해보니 social·sense의 가정이 사실상 반대였다:
//   social  가정 0.3161 → 실측 0.1341 (-0.18)
//   sense   가정 0.2383 → 실측 0.0755 (-0.16)
// 그 결과 두 기질의 z는 항상 -1.8 언저리로 눌려 Top2에 절대 들지 못했고, 도달 가능한 캐릭터가
// {주도·지략·실행·신뢰} 조합 6종으로 줄어 서로 다른 얼굴이 전부 재상상으로 몰렸다(사용자 리포트).
// 관계·감각이 필요한 10종(군왕·사신·상단주·의원·예인·개혁가·학자·개척자·장인·군자)은 아예 못 나왔다.
//
// 2차 보정 (사진 16장) — 1차는 6장이라 표준편차가 좁게 잡혀 T-score가 0/100에서 잘리는 일이 잦았다
// (drive 0점 4건 등). 잘린 값은 "매우 낮음"이 아니라 "범위 밖"이라 판정이 그만큼 거칠어진다.
// 원시 벡터(characterResult.faceRaw)를 직접 모아 계산했다 — 잘린 점수에서 역산하면 분산이 왜곡된다.
// 16장 기준으로는 여섯 기질 모두 잘림 없이 26~89 범위에 들어온다.
// 여전히 통계적으로 넉넉한 표본은 아니다. 실사용 데이터가 쌓이면 §37대로 다시 교체할 것.
// 7차 보정 (사진 49장, FACE_TRAIT_MAP 중복 기여 정리 이후) — 매핑이 바뀌면 raw 분포도 바뀌므로 재산출.
...
// 매핑 정리로 완화된 결합 (실사진 49장 기준)
//        lead↔drive      +0.47 → +0.29   (장군상 쏠림 원인)
//        stability↔sense -0.42 → +0.22   (장인상이 막히던 원인, 부호가 뒤집힘)
// 남은 결합: strategy↔social -0.59, strategy↔drive -0.51 — 사신상이 여전히 나오기 어렵다.
// strategy가 다른 기질 대부분과 음의 상관이라 "지략이 높으면 나머지가 눌리는" 구조가 남아 있다.
// 정규화 기준값은 각 기질의 위치만 옮길 뿐 이 묶임 자체는 못 푼다 — 근본 해결은
// face-trait-map.js의 FACE_TRAIT_MAP에서 같은 랜드마크가 여러 기질에 중복 기여하는 부분 정리.
